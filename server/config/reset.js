...
const seedTripsTable = async () => {
  ...
  for (const trip of tripsData) {
    ...
    try {
      ...
      console.error(`⚠️ error inserting trip: ${trip.title}`, err);
    }
  }
};
...
const createTripsDestinationsTable = async () => {
  const createTripsDestinationsQuery = `
  ...
  try {
    const res = await pool.query(createTripsDestinationsQuery);
...
const createTripsUsersTable = async () => {
  ...
  try {
    ...
    console.log("🎉 trips_users table created Sucessfully");
  } catch (error) {
